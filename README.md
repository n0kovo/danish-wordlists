...
dk_breach.txt  |  1255980  |  Huge list compiled from harvesting Danish account data from a large amount of public data breach dumps available from BreachForums, Telegram, Pastebin etc. Removed duplicates, pruned everything not in the 8-20 char. length range, removed purely numeric entries, decoded base64 encoded entries, removed email addresses and fixed entries garbled by encoding errors where possible.  |  DIY
subdomains.txt  |  18659  |  List of subdomains scraped from SSL certificates of domains from a combination of a large list of .dk domains, and a port scan of the entire Danish IP space on common HTTPS ports. Sorted by frequency of occurrence. |  DIY
